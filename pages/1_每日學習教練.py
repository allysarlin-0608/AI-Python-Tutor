...
def persist():
    try:
        ...
        st.warning(f"學習紀錄沒辦法存到檔案（{e}），記得從側邊欄下載備份。")
...
# SIDEBAR
# ============================================================
today_default = datetime.now(TIMEZONE).date()
...
with st.sidebar:
    ...
    for key, label in core.TOPICS.items():
        count = len({e["date"] for e in log["entries"] if e["topic"] == key})
        if count:
            st.write(f"{label}：{count} 次（下次 {core.level_for_session(count + 1)}）")
    if not log["entries"]:
        st.write("還沒有紀錄，今天就是第一筆。")

    st.divider()
    st.download_button(
        "下載學習紀錄備份",
        data=json.dumps(log, ensure_ascii=False, indent=2),
        file_name="learning_log.json",
    ...
    if uploaded is not None and st.button("用這份紀錄取代目前紀錄", use_container_width=True):
        try:
            st.session_state.coach_log = core.parse_log(json.load(uploaded))
...
system_prompt = core.build_system_prompt(log, topic, today)
...
if not st.session_state.coach_messages:
    ...
    if st.button("開始今天的學習", type="primary", use_container_width=True):
        ...
        with st.chat_message("user"):
            st.markdown(kickoff)
        lesson = ask_coach(system_prompt)
        if lesson:
